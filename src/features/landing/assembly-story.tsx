...
const PILLARS: Pillar[] = [
  {
    ...
    descAr:
      "تتبّع كل قطعة بالعيار والوزن الصافي والدرج، بدقة تصل إلى 0.001 جرام.",
    descEn:
      "Track every piece by karat, net weight and tray — to 0.001g precision.",
  },
  {
    icon: Coins,
    ...
    descAr:
      "أسعار العيارات بالجنيه المصري تُحدَّث لحظيًا وتُطبَّق تلقائيًا على كل عملية بيع.",
    descEn:
      "EGP karat rates update instantly and apply automatically across every sale.",
  },
  {
    icon: Users,
    ar: "إدارة العملاء",
  ...
  },
  {
    ...
    en: "Reporting",
    descAr:
      "تقارير مبيعات ومخزون وضريبة جاهزة للطباعة والتصدير في أي لحظة تحتاجها.",
  ...
  },
  {
    ...
    en: "Invoicing",
    descAr:
      "فواتير أنيقة للشغل والسبائك مع احتساب المصنعية والضريبة تلقائيًا.",
    descEn:
      "Refined invoices for jewelry and bullion with automatic making-charge and VAT.",
  },
  {
    icon: ShieldCheck,
    ar: "الأمان",
...
export function AssemblyStory({ locale }: { locale: string }) {
  ...
  return (
    <section id="why" ref={ref} className="relative">
      <GoldBars />
      <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[40rem] bg-gradient-to-b from-gold-soft/25 to-transparent" />

      <div className="mx-auto grid max-w-[86rem] gap-x-16 px-6 lg:grid-cols-2 lg:px-12">
        {/* Sticky ring box stage */}
        <div className="top-0 h-[70vh] lg:sticky lg:h-screen">
          <div className="flex h-full items-center justify-center">
            <RingBoxScene progress={progress} className="max-w-[30rem]" />
          </div>
        </div>

        {/* Pillars */}
        <div className="py-24 lg:py-[24vh]">
          <motion.div
...
function PillarCard({
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className="group relative flex items-start gap-6 overflow-hidden rounded-[2rem] border border-white/70 bg-surface/70 p-8 shadow-soft backdrop-blur-xl transition-[box-shadow,border-color] duration-500 hover:border-gold/35 hover:shadow-floating"
    >
      ...
      <span className="flex size-14 shrink-0 items-center justify-center rounded-2xl border border-border/60 bg-surface-muted/60 text-gold-deep transition-all duration-500 group-hover:border-gold/40 group-hover:bg-gold-soft">
      ...
      <span className="absolute bottom-6 end-8 font-mono text-xs text-muted-foreground/40">
